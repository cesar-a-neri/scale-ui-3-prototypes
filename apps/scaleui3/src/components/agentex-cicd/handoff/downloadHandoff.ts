// In-app handoff export for the Golden Agent prototype. Bundles the prototype's
// source + HANDOFF.md into a .zip an engineer can read top-to-bottom. The source
// is embedded verbatim by scripts/gen-handoff.mjs (runs on dev/build) into
...
//
// Scope: only the Golden Agent prototype source. ScaleUI3 components are NOT
// bundled — in production they come from the ScaleUI3 registry (see HANDOFF.md).

const ROOT = 'golden-agent-handoff';
...
/** Build the Golden Agent handoff bundle and trigger a browser download. */
export async function downloadGoldenAgentHandoff(): Promise<void> {
